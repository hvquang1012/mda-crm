...
// báo mới, cảnh báo đang mở tự đóng, link thợ + chủ nhà bị khoá (trigger trg_projects_status).
...
export async function setProjectStatus(project, close, openItems = 0) {
  const msg = close
    ? `Đóng công trình "${project.name}"?\n\nCông trình sẽ ẩn khỏi Tổng quan, không còn cảnh báo. Link của thợ và chủ nhà bị khoá. Dữ liệu, ảnh, báo cáo giữ nguyên — mở lại được bất cứ lúc nào, link cũ dùng lại được.`
      + (openItems ? `\n\n⚠ Còn ${openItems} đầu việc chưa xong.` : '')
    : `Mở lại công trình "${project.name}"? Công trình sẽ hiện lại ở Tổng quan, link thợ và chủ nhà cũ dùng lại được.`;
